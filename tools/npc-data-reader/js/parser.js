...
window.NPC_DATA_READER_PARSER_RULES.push({
  id: 'toukyu-vampire-clan-enemy-info-v1',
  version: 'v1.522',
  scenario: 'とある吸血鬼一族の純血事情',
  notes: [
    '[敵情報:名前] 形式をNPC見出しとして認識する。',
    '拳銃ダメージ:1D10 / 群れ攻撃ダメージ:1D8 / 鍵爪ダメージ:1D4+DB のような技能名+ダメージを記録する。',
    '[拳銃 80]、[群れ攻撃 50]、[回避 50] のような角括弧技能を戦闘欄へ分類する。',
    '技能名とダメージ元が一致する場合、判定行の次に 【技能名・ダメージ】 を出力する。',
    '鍵爪/鉤爪 表記揺れを同一攻撃として扱う。',
    '噛みつき処理:... を戦闘補足として保持する。'
  ]
});

window.NPC_DATA_READER_PARSER_RULES.push({
  id: 'v1.523-vampire-clan-damage-map-fix',
  version: 'v1.523',
  scenario: 'とある吸血鬼一族の純血事情',
  notes: [
    'ひらがなを含む攻撃名（例：群れ攻撃）のダメージ対応を壊さず保持する。',
    '群れ攻撃ダメージ:1D8 と [群れ攻撃 50] を紐付け、1D8 【群れ攻撃・ダメージ】として出力する。'
  ]
});

window.NPC_DATA_READER_PARSER_RULES.push({
  id: 'official-ish-gangsters-number-in-weapon-name-v1',
  version: 'v1.525',
  scenario: 'オークスとショーナシー、アトランティック・シティから来た悪漢',
  notes: [
    '射撃（32口径オートマチック拳銃）50%（25／10）、ダメージ 1D8 のように技能名の括弧内に数字を含む場合でも、32ではなく50を技能値として取得する。',
    '技能値は % 直前の数字を優先して取得する。',
    'ダメージ元が明示されている場合、1D8 【射撃（32口径オートマチック拳銃）・ダメージ】として出力する。'
  ]
});

window.NPC_DATA_READER_PARSER_RULES.push({
  id: 'v1.526-shared-skills-and-multi-damage-split',
  version: 'v1.526',
  scenario: 'オークスとショーナシー、アトランティック・シティから来た悪漢',
  notes: [
    '技能（2人で共有している）： のような括弧付き技能見出しを技能ブロック開始として認識する。',
    '技能見出しの括弧内説明を技能名に混入させない。',
    '射撃（32口径オートマチック拳銃）50% のように技能名内に数字がある場合も、%直前の値を技能値として取得する。',
    'ダメージは1D3+DB、1D3+1+DB（ブラスナックル使用時）、1D4+2+DB（スティレット・ナイフ使用時）のような複数ダメージを個別行に分割する。',
    '注釈のない先頭ダメージは式のみ、注釈つきダメージは【ダメージ（注釈）】として出力する。'
  ]
});

window.NPC_DATA_READER_PARSER_RULES.push({
  id: 'v1.527-alternative-damage-and-comma-spells',
  version: 'v1.527',
  scenario: 'アブナー・ウィック系公式NPCデータ',
  notes: [
    'もしくは1D6+DB（肉切りナイフ使用時）のような次行の代替ダメージを直前の戦闘技能へ追加する。',
    '複数ダメージは個別行へ分割し、注釈つきダメージは【ダメージ（注釈）】として出力する。',
    '呪文：A、B、C のような読点区切り呪文を複数行へ分割する。',
    '装甲：1ポイントの... のような公式風装甲文を装甲欄へ保持する。'
  ]
});

window.NPC_DATA_READER_PARSER_RULES.push({
  id: 'official-alternative-damage-spells-armor-v1-528',
  version: 'v1.528',
  scenario: 'アブナー・ウィック形式',
  notes: [
    'もしくは1D6+DB（肉切りナイフ使用時）のような代替ダメージ行を直前の戦闘技能へ紐付ける。',
    '代替ダメージの括弧注釈は「肉切りナイフ使用時・ダメージ」のようなラベルとして出力する。',
    '装甲：1ポイントのゴムめいた皮膚。を装甲欄とチャットパレットの// ▼ 装甲に保持する。',
    '呪文：A、B、C の読点区切りを// ▼ 呪文に複数行で出力する。'
  ]
});

window.NPC_DATA_READER_PARSER_RULES.push({
  id: 'v1.529-abner-wick-alt-damage-block-fix',
  version: 'v1.529',
  scenario: 'アブナー・ウィック形式',
  notes: [
    'もしくは1D6+DB（肉切りナイフ使用時）のような代替ダメージ行をNPC見出しとして誤認しない。',
    '代替ダメージ行を直前の戦闘技能へ紐付け、ダメージ（注釈）形式で出力する。',
    '代替ダメージ行以降の回避・装甲・技能・呪文ブロックを切り捨てず解析する。'
  ]
});


window.NPC_DATA_READER_PARSER_RULES.push({
  id: 'v1.532-crimson-letters-feidman-siblings',
  version: 'v1.532',
  scenario: '紅文字 -Crimson Letters- / ヘクターとカーラのフェイドマン兄妹',
  notes: [
    '1ラウンドの攻撃回数：2 のような単純な攻撃回数メモは戦闘欄へ出力しない。',
    'か1D6+DB（ハンティング・ナイフ使用時）のような「か」始まりの代替ダメージを直前の戦闘技能へ紐付ける。',
    '代替ダメージは 1D6+{DB} 【ダメージ（ハンティング・ナイフ使用時）】 のように分割出力する。'
  ]
});
