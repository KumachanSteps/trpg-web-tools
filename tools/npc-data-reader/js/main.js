...
ダメージ 1D3+DB（素手）
回避 30%（15/6）`;
let state={name:'カルティスト',title:'名もなき神の信者',nameMode:'full',system:'auto',resolved:'coc7',status:{hp:'13',mp:'10',san:'0',db:'+0',build:'0',move:'8'},abilities:{STR:'60',CON:'70',SIZ:'60',DEX:'60',INT:'50',APP:'35',POW:'50',EDU:'60'},combat:[{name:'近接戦闘（格闘）',value:'60',damage:'1D3+1+DB（ナイフ）'},{name:'首を絞める（mnvr）',value:'',damage:'1D3+DB（素手）'},{name:'回避',value:'30',damage:''}],armor:'',skills:[],spells:[],memo:'カルティスト',manualPalette:false,commandAddon:false};
...
function tailAfterStatLines(raw){
  ...
  lines.forEach((line,i)=>{const n=normalize(line); if(/^(?:[・\s]*)?(?:ステータス|6版|7版)$/.test(n)||/^(STR|CON|SIZ|DEX|INT|APP|POW|EDU)\b/i.test(n)||/^(SAN|正気度|耐久力|耐久|HP|マジックポイント|MP|ダメージボーナス|ダメボ|DB|ビルド|BLD|移動|MOV)(?:[:：\s+\-]|$)/i.test(n)) last=i;});
  return last>=0?lines.slice(last+1).join('\n'):raw;
}

function isNonSkillName(name){
  const s=normalize(name).replace(/[\s:：]/g,'');
  return /^(STR|CON|SIZ|DEX|INT|APP|POW|EDU|SAN|HP|MP|DB|正気度|耐久力|耐久|体力|マジックポイント|ダメージボーナス|ダメボ|ビルド|BLD|移動|MOV|幸運|知識|IDE|年齢|性別|身長|6版|7版)/i.test(s);
}
...
function collectLooseSkills(text){
  ...
  const add=(name,value)=>{
    name=normalize(name).replace(/^[・\-\s]+/,'').replace(/[〈〉《》【】]/g,'').trim();
  ...
  let m;
  // 同人形式: 〈回避〉 70％ / 《目星》50% / 【信用】 60%
  const bracketRe=/[〈《【]\s*([^〉》】]+?)\s*[〉》】]\s*(\d{1,3})\s*%?/g;
  while((m=bracketRe.exec(src))) add(m[1],m[2]);
  // Bracketed skill lines can contain several skills in one line. Remove them before fallback parsing
  // so the whole line is not mistakenly captured as one large skill name.
  const fallbackSrc=src.replace(/[〈《【]\s*[^〉》】]+?\s*[〉》】]\s*\d{1,3}\s*%?/g,' ');
  // 同人形式: 図書館（65%） / 製作(服飾)（80%）
  let parenRe=/([^\n、,。%〈《【]+?(?:[（(][^0-9%）)]*?[）)])?)\s*[（(]\s*(\d{1,3})\s*%?\s*[）)]/g;
...
function parseText(text){
 ...
 state.status.hp=parseNum(/(?:耐久力|耐久|HP|体力)\s*:?\s*(\d+)/i,t);
 state.status.mp=parseNum(/(?:マジックポイント|MP)\s*:?\s*([+-]?\d+)/i,t);
 state.status.db=parseNum(/(?:ダメージボーナス|ダメボ|DB)\s*:?\s*([^\s]+)/i,t);
 state.status.build=parseNum(/(?:ビルド|BLD)\s*:?\s*([+-]?\d+)/i,t).replace(/^\+/, '');
 state.status.move=parseNum(/(?:移動|MOV)\s*:?\s*([+-]?\d+)/i,t).replace(/^\+/, '');
 if(!state.status.db){ const inferred=inferDbFromBuild(state.status.build); if(inferred) state.status.db=inferred; }
 state.resolved=(/7版|MOV|BLD/.test(raw)||/ビルド|移動/.test(raw)||Object.values(state.abilities).some(v=>Number(v)>20))?'coc7':'coc6';
 state.combat=[]; state.skills=[]; state.spells=extractSpells(raw); state.armor=''; let skillPart='';
 const skillIdx=raw.search(/(?:\[\s*技能\s*\]|【\s*技能\s*】|技能[:：]|所持技能[:：]|特技[:：])/);
 const beforeSkill=skillIdx>=0?raw.slice(0,skillIdx):raw;
 if(skillIdx>=0) skillPart=raw.slice(skillIdx).replace(/^.*?(?:\[\s*技能\s*\]|【\s*技能\s*】|技能[:：]|所持技能[:：]|特技[:：])/s,'');
 ...
 if(!rows.length && /\[\s*技能\s*\]|【\s*技能\s*】/.test(raw)){ const after=raw.split(/\[\s*技能\s*\]|【\s*技能\s*】/).slice(1).join('\n'); rows=collectLooseSkills(after); }
 if(!rows.length && skillIdx<0){ rows=collectLooseSkills(tailAfterStatLines(raw)); }
...
function generatePalette(){const pre=commandPrefix(); let out=[]; if(state.commandAddon) out.push(...commandAddonLines(), ''); if(state.combat.some(r=>r.name||r.damage)){out.push('// ▼ 戦闘'); state.combat.forEach(r=>{if(r.value) out.push(`${pre}${r.value} 【${r.name}】`); else if(r.name) out.push(r.name); const dl=damageLine(r.damage); if(dl) out.push(dl);});}
...
function adjustLayout(){
  ...
  // v1.472: keep footer inside the shared work scrollbar so it never overlaps the fixed left column.
...
function resetToDefault(){state={name:'カルティスト',title:'名もなき神の信者',nameMode:'full',system:'auto',resolved:'coc7',status:{hp:'13',mp:'10',san:'0',db:'+0',build:'0',move:'8'},abilities:{STR:'60',CON:'70',SIZ:'60',DEX:'60',INT:'50',APP:'35',POW:'50',EDU:'60'},combat:[{name:'近接戦闘（格闘）',value:'60',damage:'1D3+1+DB（ナイフ）'},{name:'首を絞める（mnvr）',value:'',damage:'1D3+DB（素手）'},{name:'回避',value:'30',damage:''}],armor:'',skills:[],spells:[],memo:'カルティスト',manualPalette:false,commandAddon:false}; $('#rawInput').value=SAMPLE; render();}
