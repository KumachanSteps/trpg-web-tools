...
function renderSummary() {
  ...
  $('summaryMemo').textContent = tr(
    'summary.memo',
    '検出した{all}件のd100ロールのうち、表示対象{total}件を集計しました。検出キャラクター数は{characters}です。非表示ロール数は{hidden}件です。総ロール数{threshold}以下のキャラクターは初期状態で非表示です。通常の判定はログの判定語で集計します。正気度ロールと1D100ロールは、左パネルの統計オプションがONの時だけログ形式から推定した版の数値基準でクリファンを集計します。',
    {
      all,
      total,
      characters,
