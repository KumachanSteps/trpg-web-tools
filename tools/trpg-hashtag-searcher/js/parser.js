window.HASHTAG_PARSER = (() => {
  ...
  function buildQueryFromState(state, templates) {
    ...
    const selectedFilterQueries = state.selectedFilters
      .filter((filter) => filter !== "最新順")
      .map((filter) => (filter.startsWith("since:1") || filter === "since:3days" ? getRelativeSinceQuery(filter) : filter));
