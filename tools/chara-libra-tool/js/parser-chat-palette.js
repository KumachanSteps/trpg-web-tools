...
const CATEGORY_6E = {
  ...
  combat: [
    ...
    "日本刀", "刀", "太刀", "短刀", "木刀", "竹刀", "剣", "刀剣", "サーベル", "レイピア", "短剣", "ダガー",
    "ナイフ", "小型ナイフ", "大きなナイフ", "包丁", "棍棒", "警棒", "バット", "杖", "メイス",
    "斧", "手斧", "チェーンソー", "フレイル", "絞殺ひも", "絞殺紐", "ガロット", "鞭", "ムチ",
    "ノコギリナタ", "ナタ", "拳銃", "サブマシンガン", "ショットガン", "マシンガン", "ライフル",
    "弓", "クロスボウ", "ボウガン", "槍", "薙刀", "銃剣", "砲",
    "鬼道", "陰陽道", "陰陽術", "魔術", "呪文", "呪術", "妖術", "魔法", "退魔", "祈祷", "結界", "式神"
  ],
...
const CATEGORY_7E = {
  ...
  combat: [
    ...
    "ナイフ", "小型ナイフ", "大きなナイフ", "包丁", "棍棒", "警棒", "バット", "杖", "メイス",
    "斧", "手斧", "チェーンソー", "フレイル", "絞殺ひも", "絞殺紐", "ガロット", "鞭", "ムチ",
    "ノコギリナタ", "ナタ", "弓", "クロスボウ", "ボウガン", "槍", "薙刀", "銃剣",
    "鬼道", "陰陽道", "陰陽術", "魔術", "呪文", "呪術", "妖術", "魔法", "退魔", "祈祷", "結界", "式神"
  ],
...
const COMBAT_WEAPON_DICTIONARY = [
  "三節棍", "三節こん", "三節棍棒", "棍", "棒術", "六尺棒", "杖術", "トンファー", "ヌンチャク",
  "鎖鎌", "鎖分銅", "分銅鎖", "フレイル", "モーニングスター", "メイス", "ハンマー", "大槌", "戦槌",
  "火炎放射器", "火炎放射", "火炎瓶", "ロケットランチャー", "グレネードランチャー", "ランチャー",
  "手榴弾", "グレネード", "爆弾", "爆薬", "ダイナマイト", "地雷", "C4", "プラスチック爆弾",
  "スタンガン", "テーザー", "催涙ガス", "催涙スプレー", "催涙弾", "煙幕", "閃光弾",
  "拳銃", "ピストル", "リボルバー", "オートマチック", "マグナム", "デリンジャー",
  "ショットガン", "ライフル", "スナイパーライフル", "アサルトライフル", "サブマシンガン", "マシンガン",
  "機関銃", "軽機関銃", "重機関銃", "火縄銃", "散弾銃",
  "弓", "弓矢", "クロスボウ", "ボウガン", "スリング", "投石", "投げナイフ",
  "日本刀", "刀", "太刀", "短刀", "脇差", "木刀", "竹刀", "刀剣", "剣", "大剣", "短剣",
  "サーベル", "レイピア", "ダガー", "ナイフ", "包丁", "鉈", "ナタ", "ノコギリナタ",
  "槍", "薙刀", "銃剣", "斧", "手斧", "チェーンソー", "鞭", "ムチ", "絞殺ひも", "絞殺紐", "ガロット",
  "肉切り包丁", "アイスピック", "ツルハシ", "バール", "警棒", "バット", "金属バット", "ブラックジャック",
  "暗器", "クナイ", "手裏剣", "チャクラム", "ブーメラン"
];
...
window.ChatPaletteParser = (() => {
  ...
  function getSkillFromLine(line) {
    ...
    if (start >= 0 && end >= 0) return line.slice(start + 1, end).trim();

    const parts = parseRollCommandParts(line);
    return parts ? parts.skill.trim() : "";
  }

  function parseRollCommandParts(line) {
    const text = String(line || "").trim();
    if (!text) return null;

    const match = text.match(/^(s?CCB?|s?CC|CCB|CC|1D100|1d100|D100|d100)\s*(<=|＜=|<＝|≦|<)\s*([^\s【\[]+)\s*(?:【([^】]+)】|\[([^\]]+)\]|(.+?))?\s*$/i);
    if (!match) return null;

    const skill = String(match[4] || match[5] || match[6] || "").trim();
  ...
  function isSectionOrGeneratedMetaLine(line) {
    const compact = String(line || "").trim();
    if (!compact) return true;
    if (Object.values(SECTION_LABELS).includes(compact)) return true;
  ...
  function normalizeSkillLine(line, edition) {
    ...
    const parts = parseRollCommandParts(commandNormalized);
    if (!parts) return commandNormalized;

    const standardSkill = normalizeSkillNameForEdition(parts.skill, edition);
    return commandForEdition(edition) + parts.operator + parts.value + " 【" + standardSkill + "】";
  }

  function parseLine(line) {
    const compact = line.trim();

    if (isSectionOrGeneratedMetaLine(compact)) {
      return { type: "skip", line };
    }

    const skillFull = getSkillFromLine(line);
    const cleanSkill = normalizeSkillName(skillFull);
    const hasCommand = /^(?:s?CCB?|s?CC|CCB|CC|1D100|1d100|D100|d100)\s*(?:<=|＜=|<＝|≦|<)/i.test(compact);
    const upper = line.toUpperCase();

    if (/^:(HP|SAN)-$/i.test(compact) || /^CBRB\s*\(/i.test(compact) || /^RESB\s*\(/i.test(compact)) {
      return { type: "utility", line };
    }
  ...
  function categorize(skill, skillFull, edition) {
    ...
    if (COMBAT_WEAPON_DICTIONARY.some(word => target.includes(word))) return "combat";
  ...
  function buildOutput(text, edition) {
    ...
    for (const line of lines) {
      ...
      if (parsed.type === "skill") {
        ...
        addUnique(buckets.utility, parsed.line, seen);
    ...
    if (buckets.utility.length) {
      output.push(...buckets.utility);
    }
  ...
  function runSelfTests() {
    const tests = [
      ...
      {
        name: "6e Japanese sword variant is categorized as combat",
        actual: /🟥戦闘技能[\s\S]*CCB<=85 【日本刀：太刀】/.test(buildOutput("CCB<=85 【日本刀：太刀】", "6e")),
        expected: true
      },
      {
        name: "6e knife is categorized as combat",
        actual: /🟥戦闘技能[\s\S]*CCB<=75 【ナイフ】/.test(buildOutput("CCB<=75 【ナイフ】", "6e")),
        expected: true
      },
      {
        name: "6e kidou is categorized as combat",
        actual: /🟥戦闘技能[\s\S]*CCB<=60 【鬼道】/.test(buildOutput("CCB<=60 【鬼道】", "6e")),
        expected: true
      },
      {
        name: "6e onmyoudou is categorized as combat",
        actual: /🟥戦闘技能[\s\S]*CCB<=70 【陰陽道】/.test(buildOutput("CCB<=70 【陰陽道】", "6e")),
        expected: true
      },
      {
