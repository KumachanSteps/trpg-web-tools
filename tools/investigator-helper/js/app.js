...
${notes}`;
  ...
  function getSearchText(item) {
    const skillOptions = Array.isArray(item.skillOptions)
      ? item.skillOptions.map(skillOptionText)
      : [];
    ...
    return OCCUPATIONS
      ...
      .map((item) => ({
        ...
        score: [...state.selectedSkills].filter((skill) => {
          ...
          const skillTexts = [
            ...(item.skills || []),
            ...((item.skillOptions || []).flatMap((option) => {
              if (typeof option === "string") return [option];
              if (!option || typeof option !== "object") return [];
              return [option.label || "", ...(option.options || [])];
            })),
          ];
          return skillTexts.some((s) => aliases.some((alias) => s.includes(alias) || alias.includes(s)));
        }).length,
      }))
  ...
  function renderOccupationDetail() {
    ...
    document.getElementById("copySkillsButton")?.addEventListener("click", () => copyText((item.skills || []).join("、")));
  }
