(() => {
  ...
  function formatAllPalettes() {
    if (!pcs.length) { showToast(infoToast, "整形するPCカードがありません。"); return; }
    pcs = pcs.map(CharashiParser.formatPcPalette);
    saveCards();
    renderCards();
    showToast(infoToast, "チャットパレットを整形しました。");
  }

  function renderCards() {
    cardLane.innerHTML = "";
    pcs = pcs.map(CharashiParser.rebuildPc);
  ...
  function createCard(pc, index) {
    ...
    card.addEventListener("drop", event => reorderCards(event, index));
    return card;
  }

  function reorderCards(event, index) {
  ...
  function renderCardContent(pc) {
    return `
      ...
      <div class="pc-card-body">
        ...
        <div class="section-label">チャットパレット</div>
  ...
  function sheetLink(pc) { return pc.externalUrl ? `<a class="sheet-link" href="${escapeAttr(pc.externalUrl)}" target="_blank" rel="noopener noreferrer">🔗キャラシ</a>` : ""; }
  ...
  async function copyToClipboard(text) {
    ...
    if (navigator.clipboard && typeof navigator.clipboard.writeText === "function" && window.isSecureContext) {
      try { await navigator.clipboard.writeText(value); return true; } catch (error) { console.warn(error); }
    }
    try {
