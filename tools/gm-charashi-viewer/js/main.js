(() => {
  const STORAGE_KEY = "gm-charashi-viewer:v2";
  ...
  function importCharacterData(rawText) { hideToast(errorToast); try { const parsed = JSON.parse(rawText.trim()); const list = Array.isArray(parsed) ? parsed : [parsed]; pcs.push(...list.map(item => CharashiParser.normalizeCharacterData(item, formatPaletteToggle.checked))); saveCards(); renderCards(); } catch (error) { showToast(errorToast, error.message || "いあきゃらのコマ出力データとして読み込めませんでした。"); } }
  ...
  function bindCardButtons() { cardLane.querySelectorAll("button[data-action='delete']").forEach(button => { button.addEventListener("click", () => { pcs = pcs.filter(pc => pc.id !== button.dataset.id); saveCards(); renderCards(); }); }); cardLane.querySelectorAll("button[data-action='copy']").forEach(button => { button.addEventListener("click", async () => { const pc = pcs.find(item => item.id === button.dataset.id); if (pc) showCopyResult(button, await copyToClipboard(pc.chatPalette)); }); }); cardLane.querySelectorAll("button[data-action='copy-koma']").forEach(button => { button.addEventListener("click", async () => { const pc = pcs.find(item => item.id === button.dataset.id); if (pc) showCopyResult(button, await copyToClipboard(CharashiParser.copyableCharacterData(pc))); }); }); }
  async function copyToClipboard(text) { const value = String(text ?? ""); if (navigator.clipboard && typeof navigator.clipboard.writeText === "function" && window.isSecureContext) { try { await navigator.clipboard.writeText(value); return true; } catch (error) { console.warn(error); } } try { const textarea = document.createElement("textarea"); textarea.value = value; textarea.readOnly = true; textarea.style.position = "fixed"; textarea.style.left = "-9999px"; document.body.appendChild(textarea); textarea.focus(); textarea.select(); const copied = document.execCommand("copy"); document.body.removeChild(textarea); if (copied) return true; } catch (error) { console.warn(error); } openCopyModal(value); return false; }
