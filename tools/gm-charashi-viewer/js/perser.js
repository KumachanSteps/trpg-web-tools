...
window.ChatPaletteParser = (() => {
  ...
  function findByKeys(obj, keys, requirePalette) {
    ...
    function search(value) {
      ...
      for (const key of keys) {
        if (typeof value[key] === "string" && (!requirePalette || includesPaletteWords(value[key]))) return value[key];
      }
  ...
  function extractPaletteText(rawInput) {
    ...
    try {
      ...
      return { text: normalizeText(findByKeys(json, ["commands", "chatPalette", "chat_palette", "palette", "memo", "command"], true)), source: "json" };
    } catch (error) {
      return { text: normalizeText(trimmed), source: "text" };
    }
  }
  ...
  function countChar(text, target) { return String(text).split("").filter(ch => ch === target).length; }
  ...
  function getValueFromLine(line) {
    ...
    for (const ch of after) {
      if ((ch >= "0" && ch <= "9") || ch === "{" || ch === "}" || ch === "_" || ch === "英" || ch === "幸" || ch === "知" || ch === "ア") value += ch;
      else break;
    }
    return value;
  }
  ...
  function detectEdition(text) {
    const names = text.split(NL).map(getSkillFromLine).map(normalizeSkillName).filter(Boolean);
  ...
  function commandForEdition(edition) { return edition === "6e" ? "CCB" : "CC"; }
  function lineForSkill(edition, value, skill) { return commandForEdition(edition) + "<=" + value + " 【" + skill + "】"; }
  ...
  function parseLine(line) {
    ...
    if (skillFull && hasCommand && isAbilityRollLabel(cleanSkill)) return { type: "status", line };
    if (skillFull && hasCommand) return { type: "skill", line, skillFull, skill: cleanSkill, value: getValueFromLine(line) };
    if (["STR", "CON", "POW", "DEX", "APP", "SIZ", "INT", "EDU", "HP", "MP", "SAN"].some(stat => upper.startsWith(stat + ":") || upper.startsWith(stat + "："))) return { type: "status", line };
    if ((upper.includes("D") && hasNumber(line)) || line.includes("ダメージ") || upper.includes("DB") || upper.includes("DAMAGE")) return { type: "damage", line };
  ...
  function hasNumber(text) { return String(text).split("").some(ch => ch >= "0" && ch <= "9"); }
  ...
  function isPlainMeleeLine(line) { return normalizeSkillName(getSkillFromLine(line)) === "近接戦闘"; }
  function isSpecializedMeleeLine(line) { return normalizeSkillName(getSkillFromLine(line)).startsWith("近接戦闘："); }
  ...
  function cleanDiceRedundancy(buckets, edition) {
    if (edition !== "6e") return;
    const hasD100San = buckets.dice.some(line => line.toLowerCase().startsWith("1d100<={san}") && line.includes("【正気度ロール】"));
    if (hasD100San) {
      buckets.dice = buckets.dice.filter(line => !(line.startsWith("CCB<={SAN}") && line.includes("【正気度ロール】")));
    }
    buckets.dice = buckets.dice.filter(line => !(line.startsWith("CCB<={幸運}") && line.includes("【幸運】")));
  }
  ...
  function getAbilityBaseValueFromLine(line, statName) {
    ...
    if (normalizedSkill === statName || normalizedSkill === statName + "×5" || normalizedSkill === statName + "X5" || normalizedSkill === statName + "*5") {
  ...
  function buildInitialLines(present, edition) {
    ...
    return Object.entries(initial).filter(([skill]) => !skip.has(skill) && !present.has(skill)).map(([skill, value]) => lineForSkill(edition, value, skill));
  }
  ...
  function normalizeAbilityRoll(line, edition) {
    const skill = normalizeSkillName(getSkillFromLine(line));
    const normalized = String(skill || "").replaceAll(" ", "").toUpperCase();
    const stat = ["STR", "CON", "POW", "DEX", "APP", "SIZ", "INT", "EDU"].find(s => normalized === s || normalized === s + "×5" || normalized === s + "X5" || normalized === s + "*5");
  ...
  function runSelfTests() {
    const tests = [
      { name: "7e normalizes knife to Fighting Brawl", actual: normalizeSkillNameForEdition("ナイフ", "7e"), expected: "近接戦闘：格闘" },
      { name: "parentheses and colon normalize to Japanese colon", actual: normalizeSkillName("射撃（拳銃）"), expected: "射撃：拳銃" },
      { name: "Cthulhu Mythos skill name normalized", actual: normalizeSkillName("クトゥルフ神話"), expected: "クトゥルフ神話技能" },
      { name: "6e specialized fist drops plain fist", actual: buildOutput("CCB<=50 【こぶし】" + NL + "CCB<=50 【こぶし：パンチ】", "6e").includes("CCB<=50 【こぶし】"), expected: false },
      { name: "6e adds mother tongue from EDU base value", actual: buildOutput("EDU:17", "6e").includes("CCB<=85 【母国語：(要編集)】"), expected: true },
      { name: "mother tongue is not duplicated when already present", actual: (buildOutput("EDU:17" + NL + "CCB<=85 【母国語：日本語】", "6e").match(/母国語/g) || []).length, expected: 1 },
      { name: "7e SAN command normalizes from 1d100 to CC", actual: normalizeCommand("1d100<={SAN} 【正気度ロール】", "7e"), expected: "CC<={SAN} 【正気度ロール】" }
    ];

    for (const test of tests) console.assert(test.actual === test.expected, test.name, { actual: test.actual, expected: test.expected });
  }

  runSelfTests();

  return { extractPaletteText, detectEdition, buildOutput, normalizeSkillName, normalizeSkillNameForEdition, normalizeCommand };
})();
...
const CharashiParser = (() => {
  ...
  function formatPcPalette(pc) {
    ...
    const chatPalette = edition === "emoklore" ? formatEmoklorePalette(raw) : window.ChatPaletteParser.buildOutput(raw, edition);
    const allSkills = parseSkillsFromCommands(chatPalette, edition);
    return {
      ...pc,
      edition,
