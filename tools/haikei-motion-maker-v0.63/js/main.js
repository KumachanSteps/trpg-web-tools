(() => {
  ...
  const els = {
    ...
    exportApngBtn: document.getElementById('exportApngBtn'),
  ...
  const DEV_SAMPLE_IMAGE_PATH = './assets/sample/sample-haikyo-room-light.jpg';
  const DEV_SAMPLE_IMAGE_NAME = '廃墟ビルの部屋（明）.jpg';
  const MAX_OUTPUT_BYTES = 5 * 1024 * 1024;

  const qualitySettings = {
    light: { fps: 10, maxWidth: 960, quality: 18, apngColors: 64, webpQuality: 0.72, label: '軽量' },
    standard: { fps: 12, maxWidth: 1280, quality: 12, apngColors: 128, webpQuality: 0.82, label: '標準' },
    high: { fps: 15, maxWidth: 1920, quality: 8, apngColors: 256, webpQuality: 0.92, label: '高品質' }
  };
  ...
  function updateQualityRecommendation() {
    ...
    els.exportNote.textContent = '※ 出力ファイルはココフォリアに対応できるよう5MB以内を目安にしてください。APNGはブラウザ環境や画像サイズによって失敗しやすいため、うまくいかない場合は画質・画像サイズ・秒数を下げるか、PNG連番ZIP / WebP出力をお試しください。';
  }
...
state.image = state.images[0] || null;
  ...
  syncPreviewCanvasSize();
      ...
      setStatus(loaded.length >= 2
        ? `${loaded.length}枚の画像を読み込みました。トランジションで順番に切り替えられます。`
        : '画像を読み込みました。エフェクトを選んでプレビューできます。');
      hideDownload();
    } catch (error) {
      console.error(error);
  ...
  async function loadSampleImageForDev() {
    try {
      ...
      const file = new File([blob], DEV_SAMPLE_IMAGE_NAME, { type: blob.type || 'image/jpeg' });
      loadFiles([file]);
      setStatus('開発用サンプル画像を読み込みました。');
    } catch (error) {
  ...
  function getEffectBounds(effect) {
    switch (effect) {
      ...
      case 'drunk':
        return { maxScale: 1.04, maxX: 12, maxY: 9, padding: 0 };
      case 'breathe':
        return { maxScale: 1.04, maxX: 0, maxY: 2, padding: 0 };
      case 'panX':
        return { maxScale: 1.04, maxX: 62, maxY: 0, padding: 0 };
      case 'wave':
      ...
      case 'rise':
      case 'descend':
        return { maxScale: 1.03, maxX: 0, maxY: 145, padding: 0 };
      case 'zoomIn':
  ...
  function syncCardTabIndex(container, selector, activeButton) {
    ...
    const target = activeButton && buttons.includes(activeButton)
      ? activeButton
  ...
  function getExportSize() {
    ...
    const match = /^([0-9]+)x([0-9]+)$/.exec(selected);
    if (match) {
      return {
  ...
  function setExportButtonsDisabled(disabled) {
    els.exportApngBtn.disabled = disabled;
  ...
  async function exportWebp() {
    ...
    setExportButtonsDisabled(true);
    setStatus(`WebP生成中... 0 / ${frames} frames / ${getSizeLimitLabel()}`);
    ...
    try {
      for (let i = 0; i < frames; i++) {
        ...
        delays.push(delay);
        if (i % 4 === 0 || i === frames - 1) {
          setStatus(`WebP生成中... ${i + 1} / ${frames} frames`);
          await new Promise(resolve => setTimeout(resolve, 0));
        }
      }
      ...
      els.downloadLink.textContent = 'ダウンロード';
      els.downloadLink.hidden = false;
      setStatus(`WebP出力完了：${outputName} / ${outputSize} / ${effectLabels[state.effect]} / ${els.loopInput.checked ? 'ループ' : '非ループ'} / ${seconds}秒 / ${setting.label} / ${sizeLabel}${sizeWarning ? ` / ${sizeWarning}` : ''}`);
    } catch (error) {
      console.error(error);
  ...
  const shortcutApi = {
    ...
    exportApng: () => clickShortcutButton(els.exportApngBtn),
