...
const DICE_COMMAND_PATTERN = /(?:S?CCB\d*|S?CC\d*|S?CBR\d*|S?RESB|RESB|CBR\d*|(?<!\d)1D100|(?<!\d)D100|S1D100|SD100|D%|D％)/i;

function hasSupportedRollCommand(line){
  const text = String(line || "");
  if (!DICE_COMMAND_PATTERN.test(text)) return false;
  // Exclude bulk/raw dice such as 5D100. These are not skill checks and should not be growth candidates.
  const bulkD100 = text.match(/(^|[^\w])(\d{1,2})\s*[dDＤｄ]\s*100\b/);
  if (bulkD100 && Number(bulkD100[2]) !== 1) return false;
  return true;
}
...
function normalizeEscapedNewlines(value){
  ...
  // Session logs may be transferred through localStorage/sessionStorage as JSON strings.
  // Repeated tool-to-tool transfers can leave literal escaped newlines such as \n, \\n, or \\r\\n.
  // Normalize any run of backslashes before r/n into real line breaks, and repeat a few times
  // so already-double-escaped strings are safely restored without accumulating artifacts.
  for (let i = 0; i < 4; i += 1) {
    const next = text
      .replace(/\\+r\\+n/g, "\n")
      .replace(/\\+n/g, "\n")
...
function prepareText(raw){
  const source = normalizeEscapedNewlines(raw);
...
function parseRolls(raw){
  ...
  lines.forEach((line, index) => {
    if (!hasSupportedRollCommand(line)) return;
...
function analyze(){
  ...
  const originalRaw = rawInput?.value || "";
  const raw = normalizeEscapedNewlines(originalRaw);
  if (rawInput && raw !== originalRaw) rawInput.value = raw;
...
function buildTransferPayload(){
  const raw = normalizeEscapedNewlines($("rawInput")?.value || "");
  return {
    type: "session-log",
...
function extractLogTextFromPayload(payload){
  ...
  for (const candidate of candidates) {
    if (typeof candidate === "string" && normalizeEscapedNewlines(candidate).trim()) return normalizeEscapedNewlines(candidate);
  }
  return "";
}
...
function importSessionLogFromStorageIfRequested(){
  ...
  rawInput.value = normalizeEscapedNewlines(transfer.logText);
...
function setupFileInput(){
  ...
  fileInput.addEventListener("change", event => {
    ...
    reader.onload = () => {
      if ($("rawInput")) $("rawInput").value = normalizeEscapedNewlines(reader.result || "");
      analyze();
    };
