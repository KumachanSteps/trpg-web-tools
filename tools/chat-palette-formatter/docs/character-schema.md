...
> **実装状況（PR2 + Charaeno修正）**: `parser.js` の `canonicalizePaletteText()` が
> ロール行を `コマンド<=閾値 【技能名】` の正規形へ揃える。
> `extractPaletteText()` / `detectEdition()` / `buildOutput()` の入口で適用。
> `normalizeSkillFieldNotation()` が分野表記を整える:
> `技能（（軽トラ））` → `技能：軽トラ`（副エントリの `(( ))` 注記を分野化）、
> `技能（）` → `技能`（空分野は除去）。
> `《》` だけで書かれた保管庫の技能テーブル本体（コマンド無し）は対象外。

Parser は技能名を以下へ寄せます（`normalizeSkillName` / `normalizeSkillFieldNotation`）:

- 囲み記号を除去: `〈技能〉` `《技能》` `【技能】` `[技能]` → `技能`
- 分野区切りを日本語コロンへ: `射撃（拳銃）` `射撃(拳銃)` `射撃:拳銃` → `射撃：拳銃`
- `こぶし／パンチ`（分野・パーレンなし）→ `こぶし：パンチ`（`／`/`/` を区切りとみなす）
- 分野がある場合の `／`/`/` は複合名の一部として保持:
  `芸術／製作（（リーゼント））` → `芸術/製作：リーゼント`、
  `射撃（ライフル／ショットガン）` → `射撃：ライフル/ショットガン`
  （`COMPOUND_SKILL_NAMES` = `芸術/製作`）
- `母国語（日本語）` → `母国語：日本語`
- `クトゥルフ神話` → `クトゥルフ神話技能`（ただし値0なら `dropZeroValueNoise()` で出力しない）
- 専門技能があるとき、素の基礎技能の値0行も落とす（例: `射撃（）` 0 ＋ `射撃：拳銃` 20）
- `芸術` 系は `製作` トークンに引かれず知識技能へ分類
...
（例: いあきゃらの全技能出力、キャラッシュ、キャラクター保管所シート）。

> **実装状況（PR4）**: `buildOutput(text, edition, { initialToCategory })` の
> 既定動作として、`extractInitialValueSkills()` が
> **値が初期値と一致する技能をカテゴリバケットから抜き**、
> `buildInitialSection()` が「入力に無い初期値技能」＋「抜いた技能」を
> 初期値順に並べて `========初期値========` へ出す。
> `initialToCategory: true`（UIの「初期値もカテゴリ分け」ON）ではこの振り分けをせず、
> 全技能をカテゴリに残す。
> 常にカテゴリに残す技能: `目星 / 聞き耳 / 図書館 / 回避 / 近接戦闘`
> （`KEEP_IN_CATEGORY_AT_INITIAL`）。
> 判定は数値の閾値のみが対象（`{SAN}` 等の変数行は対象外）。
> `analyzePalette()` の `skills[].isInitial` も同じ基準。
